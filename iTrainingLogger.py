...
if __name__ == '__main__':
    ...
    with open('data/logs.txt', 'r') as f:
        for line in f.readlines():
            ...
            s = float(items[0].split(' ')[-1])
            e = float(items[1].split(' ')[-1])
            l = float(items[2].split(' ')[-1])
            sp = float(items[3].split(' ')[-2])
            step.append(s)
            epoch.append(e)
            loss.append(l)
            speed.append(sp)
    ...
    for i in range(len(step)):
        writer.add_scalar('loss', loss[i], step[i])
        writer.add_scalar('epoch', epoch[i], step[i])
        writer.add_scalar('speed', speed[i], step[i])
        writer.record()
