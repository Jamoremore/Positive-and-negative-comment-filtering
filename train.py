...
parser.add_argument("--max_seq_len", default=32, type=int,help="The maximum total input sequence length after tokenization. Sequences longer "
...
parser.add_argument("--num_labels", default=3, type=int, help="Total classes of labels.")
...
parser.add_argument("--loss_func", default='focal_loss', type=str, help="choose loss function.", choices=['cross_entropy', 'focal_loss'])
...
def train():
    ...
    dataset = dataset.map(convert_func, batched=True)  # 支持批次操作

    if args.use_class_weights:  # 每个类别按照数据量计算类别权重改善数据不均衡，当出现imbalance情况时进行少样本类别系数补偿
        class_count_dict = {}
